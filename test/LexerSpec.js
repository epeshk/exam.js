...
describe('Lexer', function() {
    ...
    describe('tryToAddSeparator()', function() {
        ...
        it('should return a InputToken', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.INPUT_TOKEN);

            expect(result[0] instanceof InputToken);
        });

        it('should return a StartBlock', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.START_BLOCK_TOKEN);

            expect(result[0] instanceof StartBlock);
        });

        it('should return a EndBlock', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.END_BLOCK_TOKEN);

            expect(result[0] instanceof EndBlock);
        });

        it('should return a StartSection', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.START_SECTION_TOKEN);

            expect(result[0] instanceof StartSection);
        });

        it('should return a EndSection', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.END_SECTION_TOKEN);

            expect(result[0] instanceof EndSection);
        });

        it('should return a EndOfLine', function() {
            var result = lexer._tryToAddSeparator([], lexer.tokens.END_OF_LINE);

            expect(result[0] instanceof EndOfLine);
        });
    });
