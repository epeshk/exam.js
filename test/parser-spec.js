...
describe('Parser tests', function(){
  describe('Simple expressions parser', function(){
    it('should return an object with a expressions', function(){
      var result = exam.parse('ТЕСТЫ\rТЕКСТ\r\rWhat is your name?\r+Exam.js\r\rКОНЕЦ ТЕСТОВ');
    ...
    it('should return a question without simple text in answers', function(){
      var result = exam.parse('ТЕСТЫ\rТЕКСТ\r\rTest question2?\r+test2\r\rКОНЕЦ ТЕСТОВ\rbla bla bla');
    ...
    it('should return an object with one type section and two questions', function(){
      var result = exam.parse('ТЕСТЫ\rТЕКСТ\r\rWhat is your name?\r+Exam.js\r\r\rTest quesion?\r+test\r\rКОНЕЦ ТЕСТОВ');
      assert.equal(result.expressions.length === 1, true);
      assert.equal(result.expressions[0].questions.length === 2, true);
    });

    it('should return an object with one type section and two questions', function(){
      var result = exam.parse('ТЕСТЫ\rТЕКСТ\r\rq1?\r+t1\r\r\rq2?\r+t2\r\rВИДЕО\r\r\rq3?\r+http://link.com\r\r\rq4?\r+http://link.com\r\rКОНЕЦ ТЕСТОВ');
      assert.equal(result.expressions.length === 1, true);
